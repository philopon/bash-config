...
__peco_select_history () {
  local BUFFER
  BUFFER=$(fc -rln 1 | peco --query "$LBUFFER" --layout=bottom-up)
  READLINE_LINE="$BUFFER"
  READLINE_POINT=${#BUFFER}
}

__peco_ghq_repository () {
  cd $(ghq list --full-path | peco --query "$LBUFFER" --layout=bottom-up)
}
...
__init_bash_config () {
  ...
  __command_exist gls && alias ls="gls --color=auto"
